...
class SlowlogController(BaseController):
    ...
    def get(self):
        ...
        for lid,timeticks,run_micro,commands,client_address,client_name  in logs:
            timestamp = datetime.datetime.fromtimestamp(int(timeticks))
            cmd=' '.join(commands)
            data['data'].append({'id':lid,'time':str(timestamp),'escapeMs':(float)(run_micro)/1000,'cmd':cmd,'client_address':client_address})
        self.write(data)